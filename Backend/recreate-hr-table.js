...
async function recreateHRUsersTable() {
  try {
    await sql.connect(config);
    console.log('✅ Connected to database\n');
    
    // Step 1: Drop existing table if it exists
    console.log('Step 1: Dropping existing hr_users table...');
    try {
      await sql.query`
        IF OBJECT_ID('dbo.hr_users', 'U') IS NOT NULL
        DROP TABLE dbo.hr_users
      `;
      console.log('✅ Dropped existing hr_users table\n');
    } catch (error) {
      console.log('⚠️  No existing table to drop\n');
    }
    
    // Step 2: Create new hr_users table
    console.log('Step 2: Creating new hr_users table...');
    await sql.query`
    ...
    `;
    console.log('✅ Created new hr_users table\n');
    
    // Step 3: Create indexes
    console.log('Step 3: Creating indexes...');
    await sql.query`CREATE INDEX IX_hr_users_email ON dbo.hr_users(email)`;
    await sql.query`CREATE INDEX IX_hr_users_is_active ON dbo.hr_users(is_active)`;
    ...
    const verifyResult = await sql.query`
      SELECT hashed_password FROM dbo.hr_users WHERE email = 'pulipatisimha@gmail.com'
    `;
